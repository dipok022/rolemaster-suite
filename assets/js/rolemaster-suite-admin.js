(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.rolemaster-suite-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.rolemaster-suite-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.rolemaster-suite-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.rolemaster-suite-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: ROLEMASTERCORE.admin_ajax,
        type: "POST",
        data: {
            action: "rolemaster_suite_recommended_activate_plugin",
            file: file,
            nonce: ROLEMASTERCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "rolemaster_suite_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: ROLEMASTERCORE.recommended_nonce
        },
