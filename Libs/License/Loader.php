...
if ( ! class_exists( 'Loader' ) ) {
	...
	final class Loader extends Client {
		...
		public function setup_refresh_event() {
			...
			register_deactivation_hook( ROLEMASTER_FILE, array( get_class( $this ), 'clear_refresh_event' ) );
		...
		public function config_page() {
			...
			<div id="<?php echo esc_attr( self::get_prefixed( 'rolemaster-suite-popup' ) ); ?>" class="rolemaster-suite-license-popup rolemaster-suite-license-settings-page
								<?php
								if ( $is_license_active ) {
									echo 'rolemaster-suite-license-status--active'; }
								?>
			">
				<div class="rolemaster-suite-license-popup--wrapper rolemaster-suite-license-popup--display">
					<div class="rolemaster-suite-license-popup--container">
						<div class="rolemaster-suite-license-popup--container-inner">

							<div class="rolemaster-suite-license-popup--notice"></div>

							<?php if ( $is_license_active ) { ?>

								<h3><?php esc_html_e( 'License Key Deactivation', 'rolemaster-suite' ); ?></h3>

								<p>
									<?php esc_html_e( 'Deactivates an License Key Key so it can be used on another blog.', 'rolemaster-suite' ); ?>
								</p>

								<table class="form-table">
									<tbody>
										<tr>
											<th scope="row"><?php esc_html_e( 'License Key', 'rolemaster-suite' ); ?></th>
										...
										<tr class="rolemaster-suite-license-status">
											<th scope="row"><?php esc_html_e( 'License Status', 'rolemaster-suite' ); ?></th>
											<td><?php echo esc_html( $license_status ); ?></td>
										</tr>
									</tbody>
								</table>

								<div class="rolemaster-suite-license-actions">
									<button class="rolemaster-suite-license-deactivate button button-primary wc-btn-active"><?php esc_html_e( 'Deactivate License', 'rolemaster-suite' ); ?></button>
							...
							<?php } else { ?>
								...
								<h3><?php esc_html_e( 'License Key Activation', 'rolemaster-suite' ); ?></h3>
								...
								<table class="form-table">
									<tbody>
										...
										<?php if ( $this->no_product_id ) { ?>
											<tr>
												<th scope="row"><?php esc_html_e( 'Product ID', 'rolemaster-suite' ); ?></th>
								...
								<div class="rolemaster-suite-license-actions">
									<button class="rolemaster-suite-license-activate button button-primary wc-btn-active"><?php esc_html_e( 'Activate License', 'rolemaster-suite' ); ?></button>
									<input type="hidden" name="nonce" value="<?php echo esc_js( wp_create_nonce( self::get_prefixed( 'admin-notice-nonce' ) ) ); ?>">
							...
							<div class="rolemaster-suite-license--loader-wrapper">
								<div class="rolemaster-suite-license--loader"></div>
							</div>
						</div>
					</div>
				</div>
		...
		public function inactive_notice() {
			...
			if ( apply_filters( 'wc_am_client_inactive_notice_override', true ) ) {
				...
				<div class="notice notice-error">
					<p><?php printf( __( 'The <strong>%1$s</strong> API Key has not been activated, so the %2$s is inactive! %3$sClick here%4$s to activate <strong>%5$s</strong>.', 'rolemaster-suite' ), esc_attr( $this->software_title ), esc_attr( $this->plugin_or_theme ), '<a href="' . esc_url( admin_url( 'admin.php?page=' . $menu_slug ) ) . '">', '</a>', esc_attr( $this->software_title ) ); ?></p>
				</div>
		...
		public function activate_license() {
			...
			if ( ! wp_verify_nonce( $nonce, self::get_prefixed( 'admin-notice-nonce' ) ) ) {
				wp_send_json_error(
					array(
						'message' => esc_html__( 'Oops! Security nonce is invalid.', 'rolemaster-suite' ),
					)
				);
			}
				...
				if ( ! empty( $activation_result ) ) {
					...
					if ( true === $activate_results['success'] && true === $activate_results['activated'] ) {
						...
						wp_send_json_success(
							array(
								'message'        => sprintf( __( '%s activated. ', 'rolemaster-suite' ), esc_attr( $this->software_title ) ) . esc_attr( "{$activate_results['message']}." ),
								'activated_text' => esc_html__( 'Activated', 'rolemaster-suite' ),
							)
						);
					}
					...
					if ( false === $activate_results && ! empty( $this->data ) && ! empty( $this->wc_am_activated_key ) ) {
						...
						wp_send_json_error(
							array(
								'message' => esc_html__( 'Connection failed to the License Key API server. Try again later. There may be a problem on your server preventing outgoing requests, or the store is blocking your request to activate the plugin/theme.', 'rolemaster-suite' ),
							)
						);
					}
					...
					wp_send_json_error(
						array(
							'message' => esc_html__( 'The License Key activation could not be commpleted due to an unknown error possibly on the store server The activation results were empty.', 'rolemaster-suite' ),
						)
					);
				}
			}
			...
			wp_send_json_error(
				array(
					'message' => esc_html__( 'License or Product ID is invalid.', 'rolemaster-suite' ),
				)
			);
		}
		...
		public function deactivate_license() {
			...
			if ( empty( $api_key ) ) {
				wp_send_json_error(
					array(
						'message' => esc_html__( 'The License Key is missing from the deactivation request.', 'rolemaster-suite' ),
					)
				);
			}
			...
			if ( 'Activated' === $activation_status ) {
				...
				if ( ! empty( $deactivation_result ) ) {
					...
					if ( true === $activate_results['success'] && true === $activate_results['deactivated'] ) {
						...
						wp_send_json_success(
							array(
								'message' => esc_html__( 'License Key deactivated. ', 'rolemaster-suite' ) . esc_attr( "{$activate_results['activations_remaining']}." ),
							)
						);
					}
			...
			wp_send_json_success(
				array(
					...
					'message' => esc_html__( 'The License Key is already deactivated.', 'rolemaster-suite' ),
				)
			);
		}
		...
		public function license_popup_link( $links ) {
			$popup_target = self::get_prefixed( 'rolemaster-suite-popup' );

			if ( 'Activated' === $this->get_activation_status() ) {
				$links['license_key'] = '<a href="#" class="rolemaster-suite-lpt-btn inactive" data-wc-popup-target="' . esc_attr( $popup_target ) . '" aria-label="' . esc_attr__( 'Settings', 'rolemaster-suite' ) . '">' . esc_html__( 'Deactivate License', 'rolemaster-suite' ) . '</a>';
			} else {
				$links['license_key'] = '<a href="#" class="rolemaster-suite-lpt-btn active" data-wc-popup-target="' . esc_attr( $popup_target ) . '" aria-label="' . esc_attr__( 'Settings', 'rolemaster-suite' ) . '">' . esc_html__( 'Activate License', 'rolemaster-suite' ) . '</a>';
			}
		...
		public function admin_scripts( $hook = '' ) {
			wp_enqueue_style( self::get_prefixed( 'rolemaster-suite-license' ), plugin_dir_url( __FILE__ ) . '/assets/license.css', null, time() );
			wp_enqueue_script( self::get_prefixed( 'rolemaster-suite-license' ), plugin_dir_url( __FILE__ ) . '/assets/license.js', array( 'jquery' ), time(), true );
			...
			wp_localize_script( self::get_prefixed( 'rolemaster-suite-license' ), 'Rolemaster_Suite_License_Manager_Vars', $localize_vars );
		}
		...
		public function view_license_popup() {
			...
			<div id="<?php echo esc_attr( self::get_prefixed( 'rolemaster-suite-popup' ) ); ?>" class="rolemaster-suite-license-popup
								<?php
								if ( $is_license_active ) {
									echo 'rolemaster-suite-license-status--active';
								}
					...
					<div class="rolemaster-suite-license-popup--container">
						<div class="rolemaster-suite-license-popup--container-inner">
							...
							<?php if ( $is_license_active ) { ?>
								...
								<h3><?php esc_html_e( 'License Deactivation', 'rolemaster-suite' ); ?></h3>

								<p>
									<?php esc_html_e( 'Deactivates an License Key so it can be used on another blog.', 'rolemaster-suite' ); ?>
								</p>
