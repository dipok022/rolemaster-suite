<?php
namespace ROLEMASTER\Libs\License;
	...
	class Client {
		...
		public function __construct( $file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '', $text_domain = '', $license_menu = array(), $inactive_notice = true ) {
			...
			if ( $this->no_product_id ) {
				...
				$this->data_key = str_replace( '-', '_', ROLEMASTER_SLUG ) . '_' . strtolower( str_ireplace( array( ' ', '_', '&', '?', '-' ), '_', $product_id ) );
			}
			...
			if ( is_admin() ) {
				...
				$this->wc_am_settings_title              = sprintf( __( '%s', 'rolemaster-suite' ), ! empty( $this->menu['page_title'] ) ? $this->menu['page_title'] : $this->software_title . ' API Key Activation', 'rolemaster-suite' );
				$this->wc_am_settings_menu_title         = sprintf( __( '%s', 'rolemaster-suite' ), ! empty( $this->menu['menu_title'] ) ? $this->menu['menu_title'] : $this->software_title . ' Activation', 'rolemaster-suite' );
				$this->wc_am_menu_tab_activation_title   = esc_html__( 'API Key Activation', 'rolemaster-suite' );
				$this->wc_am_menu_tab_deactivation_title = esc_html__( 'API Key Deactivation', 'rolemaster-suite' );
		...
		public function register_menu() {
			...
			switch ( $this->menu['menu_type'] ) {
				...
				default:
					// add_options_page( $page_title, $menu_title, $capability, $menu_slug, $callback = '', $position = null ) .
					add_options_page(
						sprintf( __( '%s', 'rolemaster-suite' ), $this->wc_am_settings_menu_title ),
						sprintf( __( '%s', 'rolemaster-suite' ), $this->wc_am_settings_menu_title ),
						'manage_options',
		...
		public function auto_update_message( $html, $plugin_file, $plugin_data ) {
			if ( $this->wc_am_plugin_name == $plugin_file ) {
				...
				if ( ! $this->get_api_key_status() || ! $this->get_api_key_status( true ) ) {
					return esc_html__( 'Auto-updates unavailable.', 'rolemaster-suite' );
				}
				...
				if ( ! empty( $plugin_data['auto-update-forced'] ) ) {
					if ( $plugin_data['auto-update-forced'] ) {
						// Forced on.
						$text = esc_html__( 'Auto-updates enabled', 'rolemaster-suite' );
					} else {
						$text = esc_html__( 'Auto-updates disabled', 'rolemaster-suite' );
					}

					$action     = 'unavailable';
					$time_class = ' hidden';
				} elseif ( in_array( $plugin_file, $auto_updates, true ) ) {
					$text       = esc_html__( 'Disable auto-updates', 'rolemaster-suite' );
					$action     = 'disable';
					$time_class = '';
				} else {
					$text       = esc_html__( 'Enable auto-updates', 'rolemaster-suite' );
					$action     = 'enable';
					$time_class = ' hidden';
				}
		...
		public function check_external_blocking() {
			...
			if ( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
				...
				if ( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
					?>
					<div class="notice notice-error">
						<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %1$s updates. Please add %2$s to %3$s.', 'rolemaster-suite' ), $this->software_title, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>' ); ?></p>
					</div>
					<?php
				}
			}
				...
				<form action='options.php' method='post'>
					<div class="main">
						...
						if ( $tab == $this->wc_am_activation_tab_key ) {
							...
							submit_button( esc_html__( 'Save Changes', 'rolemaster-suite' ) );
						} else {
		...
		public function load_settings() {
			...
			// API Key .
			add_settings_section( $this->wc_am_api_key_key, esc_html__( 'API Key Activation', 'rolemaster-suite' ), array( $this, 'wc_am_api_key_text' ), $this->wc_am_activation_tab_key );
			add_settings_field( $this->wc_am_api_key_key, esc_html__( 'API Key', 'rolemaster-suite' ), array( $this, 'wc_am_api_key_field' ), $this->wc_am_activation_tab_key, $this->wc_am_api_key_key );

			/*
			 * @since 2.3
			 */
			if ( $this->no_product_id ) {
				add_settings_field( 'product_id', esc_html__( 'Product ID', 'rolemaster-suite' ), array( $this, 'wc_am_product_id_field' ), $this->wc_am_activation_tab_key, $this->wc_am_api_key_key );
			}

			/*
			 * @since 2.8
			 */ // if ( version_compare( $wp_version, '5.5', '>=' ) ) {
			// add_settings_field( $this->wc_am_auto_update_key, esc_html__( 'Auto Plugin Updates', 'rolemaster-suite' ), array(
			// $this,
			// 'wc_am_auto_update_radio'
			// ), $this->wc_am_activation_tab_key, $this->wc_am_api_key_key );
			// } .

			add_settings_field( 'status', esc_html__( 'API Key Status', 'rolemaster-suite' ), array( $this, 'wc_am_api_key_status' ), $this->wc_am_activation_tab_key, $this->wc_am_api_key_key );
			add_settings_field( 'info', esc_html__( 'Activation Info', 'rolemaster-suite' ), array( $this, 'wc_am_activation_info' ), $this->wc_am_activation_tab_key, $this->wc_am_api_key_key );
			...
			add_settings_section( 'deactivate_button', esc_html__( 'API Deactivation', 'rolemaster-suite' ), array( $this, 'wc_am_deactivate_text' ), $this->wc_am_deactivation_tab_key );
			add_settings_field( 'deactivate_button', esc_html__( 'Deactivate API Key', 'rolemaster-suite' ), array( $this, 'wc_am_deactivate_textarea' ), $this->wc_am_deactivation_tab_key, 'deactivate_button' );
		}
		...
		public function wc_am_api_key_status() {
			if ( $this->get_api_key_status( true ) ) {
				$license_status_check = esc_html__( 'Activated', 'rolemaster-suite' );
				update_option( $this->wc_am_activated_key, 'Activated' );
				update_option( $this->wc_am_deactivate_checkbox_key, 'off' );
			} else {
				update_option( $this->wc_am_activated_key, 'Deactivated' );
				$license_status_check = esc_html__( 'Deactivated', 'rolemaster-suite' );
			}
		...
		public function wc_am_activation_info() {
			...
			if ( $this->get_api_key_status() ) {
				...
				echo esc_html__( 'Previous activation attempt errors:', 'rolemaster-suite' );
				echo Helper::wp_kses_custom( $line_break );
		...
		// echo '<input type="radio" name="' . esc_attr( $this->wc_am_auto_update_key ) . '" value="on"' . checked( get_option( $this->wc_am_auto_update_key ), 'on', false ) . '>' . esc_html__( 'On', 'rolemaster-suite' ) . '<br /><br />';
		// echo '<input type="radio" name="' . esc_attr( $this->wc_am_auto_update_key ) . '" value="off"' . checked( get_option( $this->wc_am_auto_update_key ), 'off', false ) . '>' . esc_html__( 'Off', 'rolemaster-suite' );
		// } .
				...
				if ( 'Deactivated' === $activation_status || '' === $activation_status || '' === $api_key || 'on' === $checkbox_status || $api_key !== $current_api_key ) {
					...
					if ( ! empty( $activation_result ) ) {
						...
						if ( true === $activate_results['success'] && true === $activate_results['activated'] ) {
							add_settings_error( 'activate_text', 'activate_msg', sprintf( __( '%s activated. ', 'rolemaster-suite' ), esc_attr( $this->software_title ) ) . esc_attr( "{$activate_results['message']}." ), 'updated' );
				...
				if ( ! empty( $deactivation_result ) ) {
					...
					if ( true === $activate_results['success'] && true === $activate_results['deactivated'] ) {
						if ( ! empty( $this->wc_am_activated_key ) ) {
							update_option( $this->wc_am_activated_key, 'Deactivated' );
							add_settings_error( 'wc_am_deactivate_text', 'deactivate_msg', esc_html__( 'API Key deactivated. ', 'rolemaster-suite' ) . esc_attr( "{$activate_results['activations_remaining']}." ), 'updated' );
						}
					...
					add_settings_error( 'not_deactivated_empty_response_text', 'not_deactivated_empty_response_error', esc_html__( 'The API Key activation could not be completed due to an unknown error possibly on the store server The activation results were empty.', 'rolemaster-suite' ), 'updated' );
				}
			}
		...
		public function wc_am_deactivate_textarea() {
			...
			echo '/>';
			?>
			<span class="description"><?php esc_html_e( 'Deactivates an API Key so it can be used on another blog.', 'rolemaster-suite' ); ?></span>
			<?php
		}
		...
		public function activate( $args ) {
			if ( empty( $args ) ) {
				add_settings_error( 'not_activated_text', 'not_activated_error', esc_html__( 'The API Key is missing from the deactivation request.', 'rolemaster-suite' ), 'updated' );
		...
		public function deactivate( $args ) {
			if ( empty( $args ) ) {
				add_settings_error( 'not_deactivated_text', 'not_deactivated_error', esc_html__( 'The API Key is missing from the deactivation request.', 'rolemaster-suite' ), 'updated' );
